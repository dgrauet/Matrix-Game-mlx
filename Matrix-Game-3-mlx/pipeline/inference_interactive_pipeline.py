"""Interactive inference pipeline for Matrix-Game-3 on MLX.

Ports the interactive MatrixGame3Pipeline from the PyTorch reference, removing
all distributed, async VAE, Int8 quantization, and torch.compile logic.
Single-device inference on Apple Silicon via MLX unified memory.

Unlike the batch pipeline, actions are collected interactively from the user
via stdin at each clip iteration.
"""
...
def get_current_action() -> dict:
    """Prompt the user for mouse/keyboard actions via stdin.

    Returns:
        Dictionary with ``"mouse"`` (numpy array, shape (2,)) and
        ``"keyboard"`` (numpy array, shape (6,)) entries.
    """
    CAM_VALUE = 0.1
    print()
    print("-" * 30)
    ...
    mouse_cond = np.array(CAMERA_VALUE_MAP[idx_mouse], dtype=np.float32)
    keyboard_cond = np.array(KEYBOARD_IDX[idx_keyboard], dtype=np.float32)
...
class MatrixGame3Pipeline:
    ...
    autoregressive multi-clip diffusion loop with interactive user-driven
    ...
    def __init__(
        ...
        # --- VAE (bfloat16, like the reference vae_config) ---
        logger.info("Loading VAE (%s) from %s", vae_type, model_path)
        self.vae = load_vae(
    ...
    def generate(
        ...
        """Generate video frames interactively from an input image and text prompt.

        At each clip iteration, the user is prompted for mouse/keyboard actions
        via stdin. The video is built up progressively, saving each clip's output
        ...
        current_image_mx = mx.array(current_image)
        # current_image_mx[0] is (1, H, W, C) channels-last -- VAE encode expects (T, H, W, C)
        img_cond = self.vae.encode([current_image_mx[0]])[0]  # (T, lat_h, lat_w, 48)
        ...
        current_end_frame_idx = 0
        keyboard_condition_all = None  # will be mx.array, grows each iteration
        mouse_condition_all = None     # will be mx.array, grows each iteration
        extrinsics_all = None          # numpy array, grows each iteration
        last_pose = None
        ...
        for clip_idx in range(num_iterations):
            ...
            # --- Collect interactive action from user ---
            if first_clip:
                action_frames = 57
                actions = get_current_action()
                keyboard_condition_curr = np.tile(actions["keyboard"], (action_frames, 1))
                mouse_condition_curr = np.tile(actions["mouse"], (action_frames, 1))
                first_pose = np.concatenate([np.zeros(3), np.zeros(2)], axis=0)
                # Compute poses from actions (first frame pose is all zeros)
                all_poses, last_pose = compute_all_poses_from_actions(
                    keyboard_condition_curr, mouse_condition_curr,
                ...
                keyboard_condition_curr_mx = mx.array(
                    keyboard_condition_curr, dtype=self.dtype
                ).reshape(1, action_frames, -1)
                mouse_condition_curr_mx = mx.array(
                    mouse_condition_curr, dtype=self.dtype
                ).reshape(1, action_frames, -1)
                keyboard_condition_all = keyboard_condition_curr_mx
                mouse_condition_all = mouse_condition_curr_mx
            else:
                action_frames = 40
                actions = get_current_action()
                keyboard_condition_curr = np.tile(actions["keyboard"], (action_frames, 1))
                mouse_condition_curr = np.tile(actions["mouse"], (action_frames, 1))
                poses_curr, last_pose = compute_all_poses_from_actions(
                    keyboard_condition_curr, mouse_condition_curr,
                    first_pose=last_pose, return_last_pose=True,
                )
                ...
                keyboard_condition_all = mx.concatenate(
                    [keyboard_condition_all, keyboard_condition_curr_mx], axis=1
                )
                mouse_condition_all = mx.concatenate(
                    [mouse_condition_all, mouse_condition_curr_mx], axis=1
                )
                extrinsics_all = np.concatenate(
                    [extrinsics_all, extrinsics_curr], axis=0
                )
            ...
            # --- Progressive video save: save each clip immediately ---
            video_np = np.array(video.astype(mx.float32))
            video_np = ((video_np + 1.0) * 127.5).clip(0, 255).astype(np.uint8)

            keyboard_curr_np = np.array(
                keyboard_condition_curr_mx[0].astype(mx.float32)
            )
            mouse_curr_np = np.array(
                mouse_condition_curr_mx[0].astype(mx.float32)
            )
            config_overlay = (keyboard_curr_np, mouse_curr_np)

            clip_output_path = os.path.join(
                output_dir, f"{save_name}_current_iteration_{clip_idx}.mp4"
            )
            process_video(
            ...
            logger.info(
                "Saved clip %d video (%d frames) to %s",
                clip_idx, len(video_np), clip_output_path,
            )
        ...
        # Save final concatenated video with overlays
        ...
        logger.info(
            "Saved final video with %d frames to %s", len(video_np), output_path
        )
